...
CREDENTIALS_FILE = os.path.join('steam', 'steam_temp', 'credentials.json')

# Créer le dossier 'steam_temp' s'il n'existe pas dans le dossier 'steam'
if not os.path.exists(os.path.join('steam', 'steam_temp')):
    os.makedirs(os.path.join('steam', 'steam_temp'))

# Définir les chemins des fichiers
STEAM_LIBRARY_FILE = os.path.join('steam', 'steam_temp', 'steam_library.json')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if jeux:
                ...
                # Sauvegarder les jeux dans steam/steam_library.json
                chemin_fichier = STEAM_LIBRARY_FILE
                with open(chemin_fichier, 'w', encoding='utf-8') as f:
                    json.dump(jeux, f, ensure_ascii=False, indent=4)
                print("Les informations des jeux ont été sauvegardées dans steam/steam_library.json.")
...
def generer_page_web(jeux, taille_tuile=300):
    ...
    with open('../PK_my_games_library.html', 'w', encoding='utf-8') as f:
        f.write(contenu_html)
    
    # Ouvrir le fichier HTML dans le navigateur
    webbrowser.open('../PK_my_games_library.html')
...
def main():
    ...
    if identifiants:
        ...
        if choix == '1':
            ...
            # Suppression de l'exécution de steam_images.py
